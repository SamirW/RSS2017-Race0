#!/usr/bin/env python
"""
Author: Samir Wadhwania, Martina Stadler, Jose Gomez
...
class Pursuit_Detector:
    ...
    def processImage(self, image_msg):
        if self.counter % 10 == 0:
            ...
            sliced_height = int(round(height*0.40))
            ...
            try:
                ...
                centroid_img = cv2.circle(centroid_img, (cx, sliced_height), 5, (255, 0, 0), -1)
                ...
                scaled_cx = min_x/float(width)
                # scaled_cy = (cy+sliced_height)/float(height)
                scaled_cy = float(min_y+sliced_height)/float(height)
                ...
                # if len(self.worldy_samples) == self.number_of_samples:
                    # print '3'
                self.publish_message(centroid_img, cone_color, worldx, worldy)
    ...
    def publish_message(self, centroid_img, cone_color, worldx_ave, worldy_ave):
        # worldx_ave = np.mean(self.worldx_samples)  
        # worldy_ave = np.mean(self.worldy_samples)
        #print "published from detector"
        #print worldx_ave*self.ft_to_m, worldy_ave*self.ft_to_m, cone_color
