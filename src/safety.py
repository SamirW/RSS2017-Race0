...
class Safety():
	...
	def safetyCB(self, msg):
		...
		if not backing_up:
			...
			try:
				min(left_front), min(right_front)

			except:
				return

			if min(left_front) < self.checkDistance:
				something_in_front_of_robot_left = True

			if min(right_front) < self.checkDistance:
			...
			if abs(sim_time - start_backup_time) >= self.backupTime:
				backing_up = False
